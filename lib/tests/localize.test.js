...
describe('Localize', function () {
    ...
    it('External Data German flat', function () {
        var localize = nls.config({ locale: 'de-DE' })(path.join(__dirname, '..', '..', 'src', 'tests', 'data'));
        assert.strictEqual(localize(0, null), 'Guten Tag Welt');
    });
    it('External Data German flat with extension', function () {
        var localize = nls.config({ locale: 'de-DE' })(path.join(__dirname, '..', '..', 'src', 'tests', 'data.js'));
        assert.strictEqual(localize(0, null), 'Guten Tag Welt');
    });
    it('External Data German flat with extension separate load', function () {
        nls.config({ locale: 'de-DE' });
        var localize = nls.loadMessageBundle(path.join(__dirname, '..', '..', 'src', 'tests', 'data.js'));
        assert.strictEqual(localize(0, null), 'Guten Tag Welt');
    });
    it('External Data German structured', function () {
        var localize = nls.config({ locale: 'de-DE' })(path.join(__dirname, '..', '..', 'src', 'tests', 'dataStructured'));
        assert.strictEqual(localize(0, null), 'Guten Tag Welt');
        assert.strictEqual(localize(1, null), 'Auf Wiedersehen Welt');
    });
    it('External Data German object', function () {
        var localize = nls.config({ locale: 'de-DE' })(path.join(__dirname, '..', '..', 'src', 'tests', 'dataObject'));
    ...
    it('Default data file', function () {
        var localize = nls.config({ locale: 'zh-tw' })(path.join(__dirname, '..', '..', 'src', 'tests', 'data'));
